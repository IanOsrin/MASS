...
// Featured songs endpoint for highlights section
app.get('/api/featured-songs', async (req, res) => {
  try {
    const countParam = Number.parseInt(req.query.count || '10', 10);
    const count = Number.isFinite(countParam) ? Math.max(1, Math.min(100, countParam)) : 10;

    console.log(`[FEATURED SONGS] Requesting ${count} featured songs`);

    // Query FileMaker for featured songs
    const fetchLimit = Math.max(count * 2, 50); // Fetch extra to account for filtering
    let data = [];
    let foundField = null;

    // Try each featured field candidate
    for (const field of FEATURED_FIELD_CANDIDATES) {
      const query = applyVisibility({ [field]: FM_FEATURED_VALUE });
      const payload = {
        query: [query],
      ...
      if (response.ok) {
        data = json?.response?.data || [];
        foundField = field;
        console.log(`[FEATURED SONGS] Using featured field "${field}", FileMaker returned ${data.length} records`);
        break;
      } else {
        const code = json?.messages?.[0]?.code;
        const msg = json?.messages?.[0]?.message || 'FM error';
        // Error 102 = field missing, 401 = no records found
        if (code === 102 || code === '102') {
          console.log(`[FEATURED SONGS] Featured field "${field}" not available (${code}), trying next...`);
          continue;
        }
        if (code === 401 || code === '401') {
          console.log(`[FEATURED SONGS] No featured records found with field "${field}"`);
          continue;
        }
        // Other errors should be reported
        console.error(`[FEATURED SONGS] FileMaker error: ${msg} (${code})`);
        return res.status(500).json({ ok: false, error: msg, code });
      }
    }

    if (!foundField) {
      console.log('[FEATURED SONGS] No featured songs found');
      return res.json({ ok: true, items: [] });
    }

    // Filter to only records with valid audio, artwork, visibility, and featured status
    const validRecords = data.filter(record => {
      const fields = record.fieldData || {};
      return recordIsVisible(fields) &&
             hasValidAudio(fields) &&
             hasValidArtwork(fields) &&
             recordIsFeatured(fields);
    });
    console.log(`[FEATURED SONGS] ${validRecords.length} valid featured records (filtered from ${data.length})`);

    // Take requested count
    const selected = validRecords.slice(0, count);

    // Build response items with all metadata (same format as random-songs)
    const items = selected.map(record => {
      const fields = record.fieldData || {};
      const recordId = String(record.recordId || '');
    ...
    console.log(`[FEATURED SONGS] Returning ${items.length} featured songs`);

    // Set cache headers (allow short browser cache since featured songs don't change often)
    res.setHeader('Cache-Control', 'public, max-age=300'); // 5 minutes

    return res.json({ ok: true, items });
  } catch (err) {
    console.error('[FEATURED SONGS] Error:', err);
    return res.status(500).json({ ok: false, error: 'Failed to load featured songs' });
  }
});
