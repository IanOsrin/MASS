...
app.post('/api/playlists/:playlistId/publish-to-filemaker', async (req, res) => {
  ...
  try {
    ...
    for (const track of playlist.tracks) {
      if (!track.recordId) {
        results.push({ track: track.name || 'Unknown', success: false, error: 'No recordId' });
        continue;
      }

      try {
        await fmUpdateRecord(FM_LAYOUT, track.recordId, {
          'PublicPlaylist': playlistName
        });
        results.push({ track: track.name || 'Unknown', recordId: track.recordId, success: true });
        console.log(`[MASS] ✓ Updated track "${track.name}" (${track.recordId}) with PublicPlaylist="${playlistName}"`);
      } catch (err) {
        const errMsg = err?.message || String(err);
        results.push({ track: track.name || 'Unknown', recordId: track.recordId, success: false, error: errMsg });
        console.error(`[MASS] ✗ Failed to update track "${track.name}" (${track.recordId}):`, errMsg);
      }
    }
