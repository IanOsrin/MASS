// server.js — thumbnails, album/track search, decade explore (auto-detect year field), faulty filter
...
// This is a *preferred* year field name; we will still auto-detect if it’s missing.
...
const fmContains = s => `=${String(s||'').trim()}`; // FileMaker "contains" operator
...
async function fmLayoutFields(token, base, layout) {
  ...
  // Expect r.data.response.fieldMetaData = [{ name, type, ... }, ...]
...
app.get('/api/albums', async (req, res) => {
  ...
  try {
    ...
    // resolve year field name for shaping/aggregation
    const names = await fmLayoutFields(token, base, FM_LAYOUT);
    const yearField = resolveYearFieldName(names);
...
// Explore by decade (auto-detect year field; FM range find; fallback to parse; hides faulty)
app.get('/api/explore/decade', async (req, res) => {
  ...
  try {
    ...
    // Determine which field *actually exists* on the layout
    const names = await fmLayoutFields(token, base, FM_LAYOUT);
    const yearField = resolveYearFieldName(names); // may be null if none match

    let rows = [];
    if (yearField) {
      // Prefer native FM range query on the detected field
      const rangeQuery = `${range.start}...${range.end}`;
      rows = await fmFind(token, base, FM_LAYOUT, { [yearField]: rangeQuery }, { limit: 2000 });
    }

    // Fallback: list-scan + parse when range yields nothing or no year field present
    if (!rows.length) {
      const allRows = [];
      let offset = 1, scanned = 0, batch = 500;
      while (scanned < MAX_ROWS_SCAN && allRows.length < 6000) {
        const chunk = await fmList(token, base, FM_LAYOUT, offset, batch);
        if (!chunk.length) break;
        allRows.push(...chunk);
        scanned += chunk.length;
      ...
      rows = allRows.filter(rec => {
        const y = parseYear((rec.fieldData || {})[yearField || F_YEAR_RELEASE_PREFERRED]);
        return y && y >= range.start && y <= range.end;
      });
    }

    let albums = aggregateAlbumsFromRows(rows, yearField || F_YEAR_RELEASE_PREFERRED);
    albums = albums.filter(a => !a.faulty);
    albums.sort((a,b) => (String(a.albumArtist||'').localeCompare(String(b.albumArtist||'')) || String(a.albumTitle||'').localeCompare(String(b.albumTitle||''))));
    const picked = sampleN(albums, Number(limit) || 15);
    res.json({ albums: picked });
  } catch (err) {
...
function resolveYearFieldName(layoutFieldNames) {
  ...
  // Build a candidate list: preferred first, then common alternates
  const candidates = [
  ...
  // try case-insensitive contains "year"
