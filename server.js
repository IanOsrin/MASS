...
app.get('/api/ai-search', async (req, res) => {
  try {
    ...
    // Call FileMaker script via Data API
    const scriptParam = JSON.stringify({ query });

    // Use the script endpoint with parameter
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        await ensureToken();
        const retryScriptUrl = `${FM_HOST}/fmi/data/v1/databases/${encodeURIComponent(FM_DB)}/layouts/${encodeURIComponent(FM_LAYOUT)}/script/AI_NaturalLanguageSearch?script.param=${encodeURIComponent(scriptParam)}`;
        const retryResponse = await safeFetch(retryScriptUrl, {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${fmToken}`,
            'Content-Type': 'application/json'
        ...
        if (!retryResponse.ok) {
          ...
          return res.status(500).json({ error: 'AI search failed', detail: errorText });
        }

        const retryJson = await retryResponse.json();
        const scriptResult = retryJson?.response?.scriptResult;

        if (!scriptResult) {
          return res.status(500).json({ error: 'No script result returned' });
        }
        ...
        // Get the AI-extracted criteria
        const criteria = result.criteria || {};
        console.log('[AI SEARCH] Extracted criteria (retry):', criteria);

        // Build FileMaker query using the extracted criteria
        const queries = buildSearchQueries({
          artist: criteria.artist === 'null' ? '' : (criteria.artist || ''),
          album: criteria.album === 'null' ? '' : (criteria.album || ''),
          track: criteria.track === 'null' ? '' : (criteria.track || ''),
          q: ''
        }, true);

        // Add genre, year, language to queries if present
        // For genre, create queries for both field names
        let finalQueries = queries;
        if (criteria.genre && criteria.genre !== 'null') {
          // Duplicate queries to search both genre fields
          const genreQueries = [];
          queries.forEach(q => {
            genreQueries.push({ ...q, 'Local Genre': `*${criteria.genre}*` });
            genreQueries.push({ ...q, 'Genre': `*${criteria.genre}*` });
          });
          finalQueries = genreQueries;
        }

        if (criteria.year && criteria.year !== 'null') {
          finalQueries.forEach(q => q['Year'] = criteria.year);
        }
        if (criteria.language && criteria.language !== 'null') {
          finalQueries.forEach(q => q['Language'] = `*${criteria.language}*`);
        }

        // Perform the find using FileMaker Data API
        const findPayload = {
          query: finalQueries,
          limit: 100
        };

        const findResponse = await fmPost(`/layouts/${encodeURIComponent(FM_LAYOUT)}/_find`, findPayload);
        const findJson = await findResponse.json().catch(() => ({}));
    ...
    if (!scriptResult) {
      return res.status(500).json({ error: 'No script result returned from FileMaker script' });
    }
    ...
    // Get the AI-extracted criteria
    const criteria = result.criteria || {};
    console.log('[AI SEARCH] Extracted criteria:', criteria);
    ...
    console.log('[AI SEARCH] FileMaker queries:', JSON.stringify(finalQueries, null, 2));
