    ...
    const adsColumn = document.getElementById('adsColumn');
    ...
    function setAuthMode(mode){
      ...
      if (authTitle) authTitle.textContent = authMode === 'register' ? 'Subscribe' : 'Log in';
      if (authSubmit) {
      ...
      if (authToggleText) authToggleText.textContent = authMode === 'register' ? 'Already subscribed?' : 'Need an account?';
    ...
    function updateAuthUI(){
      ...
      if (adsColumn) adsColumn.hidden = hasUser; // Show ads when NOT logged in
    ...
    function prepareNowPlayingAdd(meta){
      ...
      const playableSrc = meta.src || resolvePlayableSrc(track.mp3 || '');
      if (!track.name) return null;
      if (!playableSrc && !track.mp3) return null;
    ...
    async function loadUserPlaylists(){
      ...
      try {
        ...
        if (!playlists.length) {
          ...
          activePlaylistId = null; // Don't auto-select first playlist
        }
        /* keep collapsed by default */ playlistTracksCollapsed = playlistTracksCollapsed;
        renderPlaylistsPanel();
        return playlists;
      ...
      if (playlistTracksList) {
        ...
        tracks.forEach((track, idx) => {
          ...
          btnPlay.addEventListener('click', async () => {
            ...
            // Play immediately - don't check validation or run validation that could pause playback
    ...
    async function deleteActivePlaylist(){
      ...
      playlistsLoaded = true;
      // Don't auto-select after deletion - keep no playlist selected
      activePlaylistId = null;
    ...
    function triggerRandomSongsAutoplayNextPage(){
    ...
    const RANDOM_SONGS_TARGET = 12;
    const RANDOM_SONGS_FETCH_BATCH = 24;
    ...
    function buildRandomSongContext(song){
      ...
      const catalogueInfo = pickFieldValue(fields, ['Catalogue #', 'Catalogue']);
      ...
      const playableSrc = resolvePlayableSrc(rawMp3);
      const context = {
        song,
        fields,
        titleInfo,
    ...
    async function loadRandomSongs(options = {}) {
      ...
      console.log('[loadRandomSongs] Starting...', autoplayResume ? '(autoplay resume)' : '');
      ...
      try {
        ...
        while (playableContexts.length < RANDOM_SONGS_TARGET && attempts < RANDOM_SONGS_MAX_ATTEMPTS) {
          ...
          const url = `/api/random-songs?count=${RANDOM_SONGS_FETCH_BATCH}&_t=${timestamp}&attempt=${attempts}`;
    ...
    function renderSongsGrid(songs) {
      ...
      for (const song of songs) {
        ...
        try {
          ...
          if (audioField.value && playableSrc) {
            ...
            playBtn.dataset.playAria = 'Play track';
            playBtn.dataset.pauseAria = 'Pause playback';
      ...
      if (countEl) {
        countEl.textContent = renderedCount ? `${renderedCount} Random Songs` : 'No playable songs';
      }
      if (!renderedCount) {
        if (errorEl) {
          errorEl.hidden = false;
...
function hideLanding(){ /* no-op: placeholder removed */ }function doSearch(q){
    ...
    function createPublicPlaylistRow(track, idx, playlistId, playlistName){
      ...
      btnPlay.addEventListener('click', () => {
        ...
        // Play immediately - don't check validation or run background validation
        // Player error events will handle any playback failures
        handlePlay(btnPlay, li, li._src);
      });
    ...
    function createSharedPlaylistRow(track, idx, options = {}){
      ...
      btnPlay.addEventListener('click', async () => {
        ...
        // For shared playlists, refresh the container URL to avoid 401 errors
        const hasRecordId = track?.trackRecordId || trackId;
        if (hasRecordId) {
          const previousLabel = btnPlay.textContent;
          btnPlay.disabled = true;
          btnPlay.textContent = 'Refreshing…';
          ...
          if (refreshed?.notFound) {
            ...
            btnPlay.classList.add('btn-error');
            btnPlay.setAttribute('aria-label', `Audio unavailable for ${readable}`);
            window.alert('This track is no longer available.');
            return;
          }
        }
    ...
    player.addEventListener('ended', async () => {
      ...
      if (nextLi && nextLi._btn && nextLi._src) {
        // For user playlists and shared playlists, refresh the container URL to avoid 401 errors
        const isUserPlaylist = nextLi._playlist && !String(nextLi._playlist).startsWith('shared:') && !String(nextLi._playlist).startsWith('public:');
        const isSharedPlaylist = nextLi._playlist && String(nextLi._playlist).startsWith('shared:');
        const hasRecordId = nextLi._meta && (nextLi._meta.trackRecordId || nextLi._meta.recordId);

        if ((isUserPlaylist || isSharedPlaylist) && hasRecordId) {
          const audioField = nextLi._audioField || nextLi._meta?.audioField || '';
          const refreshed = await refreshTrackContainerSource(nextLi, {
            recordId: nextLi._meta.trackRecordId || nextLi._meta.recordId,
            audioField: audioField,
            candidates: AUDIO_FIELD_CANDIDATES,
            forceRefresh: false
          });
          ...
          if (refreshed?.notFound) {
            // Skip to next track
            const skipNext = findNextPlayableRow(nextLi);
            if (skipNext && skipNext._btn && skipNext._src) {
              handlePlay(skipNext._btn, skipNext, skipNext._src);
            }
            return;
          }
        }
      ...
      if (!nextLi && nowPlayingInfo.meta?.playlistId) {
        // Determine which container to search based on playlist type
        const playlistId = nowPlayingInfo.meta.playlistId || '';
        const isSharedPlaylist = playlistId.startsWith('shared:');
        const isPublicPlaylist = playlistId.startsWith('public:');
        const tracksContainer = isSharedPlaylist ? sharedPlaylistTracks :
                               isPublicPlaylist ? publicPlaylistTracks :
                               playlistTracksList;
        ...
        if (tracksContainer) {
          const selector = nowPlayingInfo.meta.playlistTrackId
            ...
            : null;
          let anchor = selector ? tracksContainer.querySelector(selector) : null;
          if (!anchor && currentRow && currentRow._playlist === nowPlayingInfo.meta.playlistId) {
            anchor = currentRow;
          }
          if (!anchor) {
            const first = tracksContainer.querySelector('.playlist-track');
            anchor = first || null;
          }
          if (anchor) {
            let candidate = findNextPlayableRow(anchor);
            ...
            if (candidate && candidate._btn && candidate._src) {
              // For user playlists and shared playlists, refresh the container URL to avoid 401 errors
              const isUserPlaylist = candidate._playlist && !String(candidate._playlist).startsWith('shared:') && !String(candidate._playlist).startsWith('public:');
              const isSharedPlaylist = candidate._playlist && String(candidate._playlist).startsWith('shared:');
              const hasRecordId = candidate._meta && (candidate._meta.trackRecordId || candidate._meta.recordId);

              if ((isUserPlaylist || isSharedPlaylist) && hasRecordId) {
                const audioField = candidate._audioField || candidate._meta?.audioField || '';
                const refreshed = await refreshTrackContainerSource(candidate, {
                  recordId: candidate._meta.trackRecordId || candidate._meta.recordId,
                  audioField: audioField,
                  candidates: AUDIO_FIELD_CANDIDATES,
                  forceRefresh: false
                });
    ...
    // Prompt guest users to login/subscribe after 10 minutes of browsing
    let loginPromptTimer = null;
    let loginPromptShown = false;

    function startLoginPromptTimer() {
      // Don't start timer if user is already logged in
      if (currentUser) return;
      // Don't start timer if prompt was already shown
      if (loginPromptShown) return;
      // Clear any existing timer
      if (loginPromptTimer) clearTimeout(loginPromptTimer);

      // Set timer for 10 minutes (600000 milliseconds)
      loginPromptTimer = setTimeout(() => {
        // Check again that user is still not logged in
        if (!currentUser && !loginPromptShown) {
    ...
    function cancelLoginPromptTimer() {
      if (loginPromptTimer) {
    ...
    function showLoginPrompt() {
      const message = `You've been exploring MASS for a while! 🎵\n\nSubscribe to:\n• Save your favorite tracks in playlists\n• Share playlists with friends\n• Never lose your music collection\n\nWould you like to subscribe or login now?`;

      if (window.confirm(message)) {
        // User clicked OK - show login/subscribe modal
        if (authModal) {
          authModal.hidden = false;
          // Default to subscribe tab
          if (authTabRegister) authTabRegister.click();
        }
      }
      // If user clicked Cancel, they can continue browsing
    }

    // Start the timer when page loads
    startLoginPromptTimer();

    // Cancel timer if user logs in
    const originalUpdateAuthUI = updateAuthUI;
    updateAuthUI = function() {
      const wasLoggedOut = !currentUser;
      originalUpdateAuthUI();
      // If user just logged in, cancel the prompt timer
      if (wasLoggedOut && currentUser) {
        cancelLoginPromptTimer();
      } else if (!currentUser && !loginPromptShown) {
        // User logged out - restart timer
        startLoginPromptTimer();
      }
    };
