...
function hideLanding(){ /* no-op: placeholder removed */ }function doSearch(q, options = {}){
    ...
    function renderPublicPlaylists(){
      ...
      items.forEach((item) => {
        ...
        li.dataset.playlistName = item.name || '';
        ...
        chevron.className = 'playlist-accordion-chevron';
        chevron.textContent = '▼';
        ...
        accordionContent.className = 'playlist-accordion-content';
        if (item.name && item.name.toLowerCase() === activeKey) {
          accordionContent.classList.add('expanded');
        }

        const tracksList = document.createElement('ul');
        tracksList.className = 'playlist-accordion-tracks';
        accordionContent.appendChild(tracksList);

        button.addEventListener('click', () => togglePlaylistAccordion(item.name, li, accordionContent, tracksList, item.image || button.dataset.image || ''));
        ...
        // Load tracks if this playlist is already active
        if (item.name && item.name.toLowerCase() === activeKey) {
          loadAccordionTracks(item.name, tracksList);
        }
      });
    ...
    async function togglePlaylistAccordion(playlistName, liElement, contentElement, tracksList, image) {
      const isExpanded = liElement.classList.contains('expanded');

      // Close all other accordions
      document.querySelectorAll('.public-playlist-item.expanded').forEach(item => {
        if (item !== liElement) {
          item.classList.remove('expanded');
          const content = item.querySelector('.playlist-accordion-content');
      ...
      if (isExpanded) {
        ...
        activePublicPlaylist = playlistName;

        // Load tracks if not already loaded
        if (tracksList.children.length === 0) {
          tracksList.innerHTML = '<li style="padding:10px;text-align:center;color:var(--muted);font-size:12px;">Loading tracks...</li>';
          await loadAccordionTracks(playlistName, tracksList);
        }
      }
    }

    async function loadAccordionTracks(playlistName, tracksList) {
      try {
        const key = playlistName.toLowerCase();

        // Check cache first
        const cached = publicPlaylistTracksCache.get(key);
        if (cached && Array.isArray(cached.tracks)) {
          renderAccordionTracks(cached.tracks, tracksList, playlistName);
          return;
        }
        ...
        // Cache the tracks
        publicPlaylistTracksCache.set(key, { tracks: sortedTracks });

        // Render tracks
        renderAccordionTracks(sortedTracks, tracksList, playlistName);

      } catch (err) {
        console.error('[Accordion] Failed to load tracks:', err);
        tracksList.innerHTML = '<li style="padding:10px;text-align:center;color:var(--muted);font-size:12px;">Failed to load tracks</li>';
      }
    }

    function renderAccordionTracks(tracks, tracksList, playlistName) {
      tracksList.innerHTML = '';

      if (!tracks.length) {
        tracksList.innerHTML = '<li style="padding:10px;text-align:center;color:var(--muted);font-size:12px;">No tracks found</li>';
        return;
      }

      tracks.forEach((track, idx) => {
        const li = document.createElement('li');
        li.className = 'playlist-accordion-track';
        ...
        playBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          playSingleTrack(track, idx, tracks, 'public-playlist', playlistName);
        });
