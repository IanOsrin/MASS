    ...
    function renderSongsGrid(songs, options = {}) {
      ...
      // When appending songs during shuffle play, keep only a small visible set
      if (autoplayResume && albumsEl) {
        const allCards = Array.from(albumsEl.querySelectorAll('.card'));
        const maxVisibleCards = 4; // Keep 4 cards max for clean UI
        if (allCards.length > maxVisibleCards) {
          const cardsToRemove = allCards.length - maxVisibleCards;
          console.log(`[renderSongsGrid] Removing ${cardsToRemove} old cards to maintain ${maxVisibleCards} visible`);
          let removed = 0;
          for (let i = 0; i < allCards.length && removed < cardsToRemove; i++) {
            ...
            if (allCards[i] !== currentRow && !allCards[i].classList.contains('playing') && !allCards[i].classList.contains('loading')) {
              allCards[i].remove();
              removed++;
              console.log(`[renderSongsGrid] Removed old card at index ${i}`);
            }
          }
        }
      }
