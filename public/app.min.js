    ...
    const nowPlayingInfoButton = document.getElementById('nowPlayingInfo');
    const nowPlayingMetadata = document.getElementById('nowPlayingMetadata');
    ...
    (function initTheme(){
      let saved = false;
      try {
        saved = localStorage.getItem(THEME_STORAGE_KEY) === '1';
      } catch {}
      applyTheme(saved);
    })();
    ...
    async function loadFeaturedAlbums(options = {}) {
      ...
      console.log('[loadFeaturedAlbums] Starting...', refresh ? '(refresh)' : '');
      if (randomSongsAutoplay.active) {
        stopRandomSongsAutoplay();
      }
      const params = new URLSearchParams();
      params.set('limit', String(FM_FETCH_LIMIT));
    ...
    function renderSongsGrid(songs) {
      ...
      for (const song of songs) {
        ...
        try {
          ...
          // Add status label for first two cards
          if (renderedCount === 0 || renderedCount === 1) {
            const statusLabel = document.createElement('div');
            statusLabel.className = 'random-song-status';
            statusLabel.style.cssText = 'font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px; opacity: 0.7;';
            statusLabel.textContent = renderedCount === 0 ? '▶ Now Playing' : '⏭ Up Next';
...
function hideLanding(){ /* no-op: placeholder removed */ }function doSearch(q){
    ...
    function renderSharedPlaylistView(){
      ...
      // Show joke ads panel to entice subscription when viewing shared playlist
      const jokeAdsPanel = document.getElementById('jokeAdsPanel');
      const playlistColumn = document.getElementById('playlistColumn');
      if (!currentUser && jokeAdsPanel && playlistColumn) {
        playlistColumn.hidden = false;
        jokeAdsPanel.hidden = false;
        // Hide playlists panel when viewing shared playlist as non-user
        const playlistsPanel = document.getElementById('playlistsPanel');
        if (playlistsPanel) playlistsPanel.hidden = true;
      }
    ...
    async function openTracksModal(album, sourceCard=null){
      ...
      let fullAlbum = album;
      const shouldFetchComplete = currentMode === 'explore' || currentMode === 'landing' || currentMode === 'featured';
    ...
    if (nowPlayingInfoButton && nowPlayingMetadata) {
      nowPlayingInfoButton.addEventListener('click', () => {
        const isHidden = nowPlayingMetadata.style.display === 'none';
        nowPlayingMetadata.style.display = isHidden ? 'block' : 'none';
        nowPlayingInfoButton.textContent = isHidden ? '×' : '⋯';

        // Populate metadata if showing
        if (isHidden && nowPlayingInfo && nowPlayingInfo.meta) {
          const meta = nowPlayingInfo.meta;
          const metadataItems = [];

          if (meta.albumTitle) metadataItems.push(`<div><strong>Album:</strong> ${meta.albumTitle}</div>`);
          if (meta.catalogue) metadataItems.push(`<div><strong>Catalogue:</strong> ${meta.catalogue}</div>`);
          if (meta.language) metadataItems.push(`<div><strong>Language:</strong> ${meta.language}</div>`);
          if (meta.producer) metadataItems.push(`<div><strong>Producer:</strong> ${meta.producer}</div>`);
          if (meta.genre) metadataItems.push(`<div><strong>Genre:</strong> ${meta.genre}</div>`);
          if (meta.composers && meta.composers.length > 0) {
            metadataItems.push(`<div><strong>Composer(s):</strong> ${meta.composers.join(', ')}</div>`);
          }
          if (meta.isrc) metadataItems.push(`<div><strong>ISRC:</strong> ${meta.isrc}</div>`);

          nowPlayingMetadata.innerHTML = metadataItems.length > 0
            ? metadataItems.join('')
            : '<div style="opacity:0.6;">No additional metadata available</div>';
        }
      });
    }
    ...
    async function runExplore(startYear, options = {}){
      ...
      params.set('start', String(start));
      params.set('end',   String(end));
      params.set('limit', '300'); // Request more tracks for greater album variety
      if (options.refresh) {
    ...
    window.addEventListener('mass:access-ready', function (e) {
