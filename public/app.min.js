    ...
    if (nextRow && nextRow._btn && nextRow._src) {
      ...
      // Check if we need to load more songs (only if we're running low)
      const remainingCards = albumsEl ? albumsEl.querySelectorAll('.card').length : 0;
      if (remainingCards <= 2 && !randomSongsAutoplay.loadingNextPage) {
        console.log('[advanceRandomSongsAutoplayFrom] Running low on cards, loading more');
        triggerRandomSongsAutoplayNextPage();
      }
    ...
    function renderSongsGrid(songs, options = {}) {
      ...
      // When appending songs during shuffle play, keep a reasonable buffer but trim excess
      if (autoplayResume && albumsEl) {
        const allCards = Array.from(albumsEl.querySelectorAll('.card'));
        const maxBufferCards = 6; // Keep 6 cards max (current + next few + buffer)
        if (allCards.length > maxBufferCards) {
          const cardsToRemove = allCards.length - maxBufferCards;
          console.log(`[renderSongsGrid] Removing ${cardsToRemove} old cards to maintain buffer of ${maxBufferCards}`);
          let removed = 0;
          for (let i = 0; i < allCards.length && removed < cardsToRemove; i++) {
            // Don't remove the currently playing card or any active card
            if (allCards[i] !== currentRow && !allCards[i].classList.contains('playing') && !allCards[i].classList.contains('loading')) {
              allCards[i].remove();
              removed++;
            }
          }
