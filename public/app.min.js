...
function hideLanding(){ /* no-op: placeholder removed */ }function doSearch(q, options = {}){
    ...
    function renderAlbumPage(){
      ...
      // In explore mode, show one page of 9 albums based on exploreCurrentPage
      if (currentMode === 'explore' || currentMode === 'landing' || currentMode === 'featured') {
        ...
        // Pad to 9 slots for consistent layout in search mode
    ...
    async function runExplore(startYear, options = {}){
      ...
      params.set('start', String(start));
      params.set('end',   String(end));
      params.set('limit', '500'); // Load 500 tracks at a time (6.4% pass rate = ~32 albums per fetch)
    ...
    function updateLoadMoreButton(hasMore, currentCount, total) {
      ...
      // In explore mode: Show Load More if there are more pages OR API has more data
      ...
      const currentPageNum = (currentMode === 'explore' || currentMode === 'landing' || currentMode === 'featured')
        ...
        : albumPage + 1;
      const totalPagesEstimate = total ? Math.ceil(total / ALBUMS_PER_PAGE) : '?';
      info.textContent = `Page ${currentPageNum} of ${totalPagesEstimate} • ${total ? total.toLocaleString() : 'many'} total albums`;
    }
