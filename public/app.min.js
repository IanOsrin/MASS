...
function hideLanding(){ /* no-op: placeholder removed */ }function doSearch(q, options = {}){
    ...
    async function runGenreExplore(genre, options = {}) {
      ...
      try {
        ...
        // Calculate if there's more data (API doesn't return hasMore for search)
        const returned = j.items ? j.items.length : 0;
        genreHasMore = returned >= 400; // If we got full limit, there might be more
        genreTotal = j.total || 0;
        genreOffset += returned;
